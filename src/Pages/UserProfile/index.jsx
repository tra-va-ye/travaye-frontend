...
const UserProfile = () => {
  ...
  const logUserVisit = async (url) => {
    const res = await fetch(
      `${process.env.REACT_APP_SERVER_URL}locations/visits?location=`,
      {
        method: 'PUT',
        headers: {
      ...
      .then((devices) => {
        if (devices && devices.length) {
          ...
          html5QrCode.start(
            ...
            (decodedText) => {
              if (decodedText === 'https://qr.me-qr.com/WiGEwIwk') {
  ...
  return (
    <Container>
      ...
      <Main>
        <button
          className='fixed right-7 bottom-6 shadow-md rounded-full z-50 !scale-90'
        ...
        <div className='d-flex justify-content-between align-items-center mb-5 mt-3'>
          <div className='flex justify-start items-center gap-2 flex-wrap'>
            <Link to='/create-event'>
              <Button color='green'>Create an Event</Button>
            </Link>
            <Link to='/plan-a-trip'>
